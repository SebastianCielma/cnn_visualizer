...
from ..config import IMAGE_SIZE, image_transform
...
class GradCAMVisualizer:
    ...
    def __init__(self, model_manager: ModelManager) -> None:
        ...
        self._cam: GradCAM = self._create_cam()

    def _create_cam(self) -> GradCAM:
        """Create GradCAM instance for current model."""
        target_layer = self.model_manager.get_gradcam_layer()
        return GradCAM(model=self.model_manager.model, target_layers=[target_layer])

    def _rebuild_cam(self) -> None:
        """Rebuild the GradCAM instance for current model."""
        self._cam = self._create_cam()
